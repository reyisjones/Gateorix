...
function Navbar({
  page, setPage, theme, toggleTheme, user, onLogout,
}: {
...
function HomePage() {
  ...
  const handleGreet = async () => {
    ...
    try {
      ...
      setError(isTauri ? `Sidecar error: ${err}` : "Cannot reach backend. Is the Go bridge running? (go run main.go --http)");
  ...
  return (
    <div className="main-content">
      ...
      <p className="page-subtitle">A React frontend + Go backend desktop app.</p>
...
function ProfilePage({ onBack }: { onBack: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await tauriInvoke("save_settings", { data: { profileName: displayName, profileEmail: email } });
      setSaved(true);
  ...
  return (
    <div className="main-content">
      ...
      <div className="form-group">
        ...
        <input className="form-input" type="text" placeholder="Your name" value={displayName}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isTauri) {
        ...
        if (username === "admin" && password === "gateorix") onLogin("Admin");
...
function App() {
  ...
  useEffect(() => {
    ...
    tauriInvoke<Record<string, unknown>>("get_settings").then((s) => {
      if (s.theme === "dark" || s.theme === "light") {
        ...
        document.documentElement.setAttribute("data-theme", s.theme as string);
      }
    });
  }, []);
  ...
  const handleLogin = (name: string) => { setUser(name); setPage("home"); };
  const handleLogout = async () => {
    if (isTauri) await tauriInvoke("logout");
